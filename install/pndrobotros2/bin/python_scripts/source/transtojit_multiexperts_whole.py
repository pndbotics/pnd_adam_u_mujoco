...
class actor_twoexperts(nn.Module): 
    def __init__(self):
        ...
        self.net1 = actor_network(num_actor_obs=91,
                                    ...
                                    activation='elu',
                                    init_noise_std=1.0)
        path_stand = './stand_model_4400.pt'
        ...
        self.net2 = actor_network(num_actor_obs=91,
                                    ...
                                    activation='elu',
                                    init_noise_std=1.0)
        path_walk = './walk_amp_PRL_0926_6400.pt'
        loaded_dict_walk = torch.load(path_walk, map_location=torch.device('cpu'))
        self.net2.load_state_dict(loaded_dict_walk['model_state_dict'])
...
class SonnyPolicy():
    ...
    def transtojit(self):
        ...
        traced_script_module.save("rl_0926_whole_libtorch.pt")
...
def main():
    ...
    model = torch.jit.load("./rl_0926_whole_libtorch.pt")
    print(model)
    obs = np.zeros(91)
    obs_1 = torch.tensor(obs, dtype=torch.float, device='cpu', requires_grad=False) 
    print(obs_1.shape)
