...
class actor_twoexperts(nn.Module): 
    def __init__(self):
        ...
        self.net1 = actor_network(num_actor_obs=91,
                                    ...
                                    activation='elu',
                                    init_noise_std=1.0)
        path_walk = 'stand_pndone_25000.pt'
        loaded_dict_walk = torch.load(path_walk, map_location=torch.device('cpu'))
        self.net1.load_state_dict(loaded_dict_walk['model_state_dict'])
        self.net2 = actor_network(num_actor_obs=91,
                                    ...
                                    activation='elu',
                                    init_noise_std=1.0)
        path_stand = './walk_amp_pndone_15000.pt'
        loaded_dict_stand = torch.load(path_stand, map_location=torch.device('cpu'))
        self.net2.load_state_dict(loaded_dict_stand['model_state_dict'])
...
class SonnyPolicy():
    ...
    def transtojit(self):
        ...
        traced_script_module.save("source/rl_1017_whole_libtorch.pt")
...
def main():
    ...
    model = torch.jit.load("source/rl_1017_whole_libtorch.pt")
    print(model)
    obs = np.zeros(91)
    obs_1 = torch.tensor(obs, dtype=torch.float, device='cpu', requires_grad=False) 
    print(obs_1.shape)
